...
class SelectShortcutTVC: DogeChatViewController, DogeChatVCTableDataSource, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        navigationItem.title = NSLocalizedString("quickAction", comment: "")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case Self.namesAndPasswords.count:
            ...
            cell.textLabel?.text = NSLocalizedString("addNewShortcut", comment: "")
        default:
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        alert = UIAlertController(title: NSLocalizedString("enterUsernameAndPassword", comment: ""), message: nil, preferredStyle: .alert)
        ...
        if indexPath.row == Self.namesAndPasswords.count {
            guard Self.namesAndPasswords.count <= 4 else {
                let warning = UIAlertController(title: NSLocalizedString("exceedQuantity", comment: ""), message: nil, preferredStyle: .alert)
            ...
            alert.addAction(UIAlertAction(title: NSLocalizedString("complete", comment: ""), style: .default, handler: { (action) in
            ...
            alert.addAction(UIAlertAction(title: NSLocalizedString("modify", comment: ""), style: .default, handler: { (action) in
                if let username = alert.textFields?[0].text, let password = alert.textFields?[1].text {
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let mainAccount = UIContextualAction(style: .normal, title: NSLocalizedString("mainAccount", comment: "")) { action, view, completion in
