...
@objc class VideoFrameData: NSObject, Codable {
    ...
    var inverseProjectionMatrix: Matrix?
    ...
    @objc func makeSampleBuffer() -> CMSampleBuffer {
        // Create a format description from the parameter sets.
        do {
            var formatDescription: CMFormatDescription?
//            formatDescription = try? CMFormatDescription(hevcParameterSets: parameterSets)
            if formatDescription == nil {
                formatDescription = try CMFormatDescription(h264ParameterSets: parameterSets)
            }
            // Get the buffer data.
